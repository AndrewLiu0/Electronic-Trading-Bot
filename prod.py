...
def lowestSellPrice(instrument):
    if(instrument in sellBook and len(sellBook[instrument]) > 0):
        return sellBook[instrument][0][0]

outstanding_orders = set()
def update_orders(id, direction):
    if direction == "add":
        outstanding_orders.add(id)
    else:
        outstanding_orders.remove(id)

def update_book(book_message):
    symbol = book_message['symbol']
    sellBook[symbol] = book_message['sell']
    buyBook[symbol] = book_message['buy']

# def pennying(state_manager):
    

def arbitrage(state_manager, trade_message):
    ...
    if 'VALE' in sellBook and (len(sellBook['VALE'])) > 0:
        vale_sell_price = sellBook['VALE'][0][0]
        vale_sell_amount = sellBook['VALE'][0][1]
    if 'VALE' in buyBook and (len(buyBook['VALE'])) > 0:
        vale_buy_price = buyBook['VALE'][0][0]
        vale_buy_amount = buyBook['VALE'][0][1]
    if 'VALBZ' in sellBook and len(sellBook['VALBZ']) > 0:
        valbz_sell_price = sellBook['VALBZ'][0][0]
        valbz_sell_amount = sellBook['VALBZ'][0][1]
    if 'VALBZ' in buyBook and len(buyBook["VALBZ"]) > 0:
        valbz_buy_price = buyBook['VALBZ'][0][0]
        valbz_buy_amount = buyBook['VALBZ'][0][1]

    # vale_sell_price = state_manager.open_and_pending_orders_in_symbol_and_direction_by_price_level("VALE", "SELL")
    # vale_buy_price = state_manager.open_and_pending_orders_in_symbol_and_direction_by_price_level("VALE", "BUY")
    # valbz_sell_price = state_manager.open_and_pending_orders_in_symbol_and_direction_by_price_level("VALBZ", "SELL")
    # valbz_buy_price = state_manager.open_and_pending_orders_in_symbol_and_direction_by_price_level("VALBZ", "BUY")
    ...
    if(vale_buy_price - valbz_sell_price > 10):
        amount = min(vale_buy_amount, valbz_sell_amount)
        state_manager.send_order("VALBZ", "BUY", valbz_sell_price, amount)
        exchange.send_convert_message(state_manager.next_order_id(), "VALE", "BUY", amount) # get a vale
        state_manager.send_order("VALE", "SELL", vale_buy_price, amount)
    elif(valbz_buy_price - vale_sell_price > 10):
        amount = min(valbz_buy_amount, vale_sell_amount)
        state_manager.send_order("VALE", "BUY", vale_sell_price, amount)
        exchange.send_convert_message(state_manager.next_order_id(), "VALE", "SELL", amount) # get a valbz
        state_manager.send_order("VALBZ", "SELL", valbz_buy_price, amount)

    # converts VALE and VALBZ to ensure that trading can still continue with VALE and VALBZ
    '''vale_position = state_manager.position_for_symbol("VALE")
    valbz_position = state_manager.position_for_symbol("VALBZ")
    if (vale_position == 10 and valbz_position == -10):
        exchange.send_convert_message(state_manager.next_order_id(), "VALE", "SELL", 10)
    elif (vale_position == -10 and valbz_position == 10):
        exchange.send_convert_message(state_manager.next_order_id(), "VALBZ", "SELL", 10)'''
...
# input sell if you want to buuy, input buy if you want to sell
def buySellPrice(instrument, quantity, direction):
    instrumentPrices = None
    if(direction == "BUY" and instrument in buyBook):
        instrumentPrices = buyBook[instrument]
    elif(direction == "SELL" and instrument in sellBook):
        instrumentPrices = sellBook[instrument]
    if(instrumentPrices == None):
        return -1
    # print("dictionary", instrumentPrices)
    currQuant =  0
    totalPrice = 0
    priceArr = []

    i = 0
    # print(instrumentPrices)

    while(quantity > currQuant):
        # print(i)
        quantNeeded = quantity - currQuant
        # print("quantity: ", quantity)
        # print("quantNeeded: ", quantNeeded)
        # print("currQuant: ", currQuant)

        if(i >= len(instrumentPrices)):
            return None
        price = instrumentPrices[i][0]
        quantAvailable = instrumentPrices[i][1]
        
        if(quantNeeded > quantAvailable):
            totalPrice += quantAvailable * price
            priceArr += [price] * quantAvailable
            currQuant += quantAvailable
            i +=1
        elif(quantNeeded <= quantAvailable):
            totalPrice += quantNeeded * price
            priceArr += [price] * quantNeeded
            currQuant += quantNeeded
            i+=1

    # print("bought:" , currQuant)
...
def break_etf(state_manager):
    def arrSum (arr):
        if (arr == -1 or arr == None):
            return 0
        sum = 0
        for prices in arr:
            sum += prices
    ...
    xlfArr = buySellPrice("XLF", 10, "SELL")
    gsArr = buySellPrice("GS", 2 , "BUY")
    msArr = buySellPrice("MS", 3, "BUY")
    wfcArr = buySellPrice("WFC", 2, "BUY")
    

    xlfPrice = arrSum(xlfArr)
    gsPrice = arrSum(gsArr)
    msPrice = arrSum(msArr)
    wfcPrice = arrSum(wfcArr)


    if(xlfPrice == 0):
        xlfPrice = 10000000000000
    ...
    GSLow = lowestSellPrice("GS")
    MSLow = lowestSellPrice("MS")
    WFCLow = lowestSellPrice("WFC")
    if(GSLow == None):
        GSLow = 0
    if (MSLow == None):
        MSLow = 0
    ...
    totalSellPrice = GSLow * 2 + MSLow * 3 + WFCLow * 2 + 3000 - 100
    print("Sell Price: " , totalSellPrice)

    print("Profit " , totalSellPrice - xlfPrice)

    if(xlfPrice < totalSellPrice):
        args = parse_arguments()
        exchange = ExchangeConnection(args=args)
        exchange.send_convert_message(state_manager.next_order_id(), "XLF", "BUY", 10)
        for price in xlfArr:
            state_manager.send_order("XLF", "BUY", price, 1)
        for price in gsArr:
            state_manager.send_order("GS", "SELL", GSLow - 1, 1)
        for price in msArr:
            state_manager.send_order("MS", "SELL", MSLow - 1 ,1)
        for price in wfcArr:
            state_manager.send_order("WFC", "SELL",WFCLow - 1, 1)
    ...
    # state_manager.send_order("XLF", "BUY", 999, 100)
...
def on_book(state_manager, book_message):
    ...
    update_book(book_message)
    arbitrage(state_manager, book_message)
    break_etf(state_manager)
    pass


def on_fill(state_manager, fill_message):
    """Called when one of y"our orders is filled."""
    if (fill_message["symbol"] == "BOND" and fill_message["dir"] == "BUY"):
        state_manager.send_order("BOND", "BUY", 999, fill_message["size"])
    elif (fill_message["symbol"] == "BOND" and fill_message["dir"] == "SELL"):
        state_manager.send_order("BOND", "SELL", 1001, fill_message["size"])
        
    if (fill_message["symbol"] == "VALE" or fill_message["symbol"] == "VALBZ"):
        print("order filled")
    pass
